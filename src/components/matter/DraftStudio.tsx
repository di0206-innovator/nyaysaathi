...
export function DraftStudio({ drafts, onNoticeDispatched }: DraftStudioProps) {
  ...
  return (
    <div className="space-y-6" id="drafts">
      <div className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm">
        ...
        {/* Lawyer-Ready Review Warning Seal */}
        {(activeDraft.communicationTier === 'lawyer_ready' || activeDraft.requiresAdvocateReview) && (
          <div className="mb-4 p-3 rounded-xl bg-purple-50/90 border border-purple-200 text-purple-950 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-2xs">
            <div className="flex items-center space-x-2.5">
              <span className="px-2 py-0.5 rounded bg-purple-200 text-purple-900 font-bold text-[10px] uppercase tracking-wider">
                Review Before Sending
              </span>
              <span className="font-medium text-[11px]">
                Statutory legal notice / court plaint draft. Recommended to verify final clauses with an enrolled Advocate or free DLSA legal aid counsel.
              </span>
            </div>
            <span className="text-[11px] font-semibold text-purple-700 bg-purple-100/80 px-2 py-0.5 rounded self-start sm:self-auto">
              Advocates Act 1961
            </span>
          </div>
        )}
        ...
        <div className="bg-stone-50 rounded-lg p-3.5 border border-stone-200 mb-4 space-y-2 text-xs">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              {activeDraft.auditLog && activeDraft.auditLog.length > 0 && (
                <span className="inline-flex items-center space-x-1 text-[11px] font-medium text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-300">
                  ...
                  <span>Audited & Neutralized ({activeDraft.auditLog.length})</span>
                </span>
              )}
        ...
        {/* Notice Execution Progression Flow */}
        <div className="mb-5 p-3.5 bg-stone-50 border border-stone-200 rounded-xl space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold text-stone-800 uppercase tracking-wider text-[11px]">
              Notice Execution Progression
            </span>
            <span className="text-[11px] text-stone-500">
              Step {markedSent ? '4 of 4' : downloaded ? '3 of 4' : '2 of 4'}
            </span>
          </div>
          <div className="grid grid-cols-4 gap-2 text-[11px]">
            <div className="p-2 rounded bg-emerald-50 border border-emerald-200 text-emerald-900 font-semibold flex items-center space-x-1">
              <Check className="w-3.5 h-3.5 text-emerald-600" />
              <span>1. Draft Prepared</span>
            </div>
            <div className="p-2 rounded bg-emerald-50 border border-emerald-200 text-emerald-900 font-semibold flex items-center space-x-1">
              <Check className="w-3.5 h-3.5 text-emerald-600" />
              <span>2. Safety Audited</span>
            </div>
            <div className={`p-2 rounded border font-semibold flex items-center space-x-1 ${
              downloaded ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : 'bg-white border-stone-200 text-stone-600'
            }`}>
              {downloaded ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Clock className="w-3.5 h-3.5 text-stone-400" />}
              <span>3. Finalized/Copied</span>
            </div>
            <div className={`p-2 rounded border font-semibold flex items-center space-x-1 ${
              markedSent ? 'bg-emerald-50 border-emerald-200 text-emerald-900' : 'bg-white border-stone-200 text-stone-600'
            }`}>
              {markedSent ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Send className="w-3.5 h-3.5 text-stone-400" />}
              <span>4. Dispatched (RPAD)</span>
            </div>
          </div>
          {!markedSent ? (
            <div className="flex items-center justify-between pt-1">
              <p className="text-[11px] text-stone-500">
                Dispatched this notice via Speed Post / Email? Mark it sent to activate response tracking.
              </p>
              <button
                type="button"
                onClick={() => {
                ...
                className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold rounded shadow-xs transition-colors"
              >
